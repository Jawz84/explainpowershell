...
if (-not (Get-Module -ListAvailable powershell-yaml)) {
    Install-Module powershell-yam -Force
}
...
$latestDotnetSdkImage = $availableDotnetSdkDockerImages | Select-Object -Last 1
...
Get-ChildItem *.csproj -Recurse -Depth 2| ForEach-Object {
    $file = $_
    $xml = [xml](Get-Content $file)
    ...
    for ($i = 0; $i -lt $xml.project.ItemGroup.PackageReference.Length; $i++) {
        ...
        if ($package.Include -in $targetVersions.PackageName) {
            $package.Version = ( $targetVersions | Where-Object PackageName -EQ $package.Include ).LatestVersion.ToString()
        }
    }
